...
function logWithTimestamp(message: string) {
  ...
  fs.appendFileSync(path.join(logsDir, 'imran_retweet.log'), logMessage + '\n');
}
...
// Function to retweet an Imran Khan tweet
async function retweetImranKhanTweet(browser: puppeteer.Browser): Promise<void> {
  logWithTimestamp('Starting Imran Khan retweet operation');
  ...
  try {
    ...
    await saveScreenshot(page, 'imran_retweet_initial.png');
    ...
    try {
      ...
      await saveScreenshot(page, 'imran_retweet_profile.png');
      ...
      await saveScreenshot(page, 'before_retweet.png');
      ...
      // Take screenshot after retweet button clicked
      await saveScreenshot(page, 'after_retweet_click.png');
      ...
      // Take screenshot after retweet confirmed
      await saveScreenshot(page, 'after_retweet_confirmed.png');
      
      logWithTimestamp('✅ Successfully retweeted an Imran Khan tweet');
      
    } catch (error: any) {
      logWithTimestamp(`Error during Imran Khan retweet operation: ${error.message}`);
      await saveScreenshot(page, 'imran_retweet_error.png');
      throw error;
    }
  } catch (error: any) {
    logWithTimestamp(`Failed to retweet Imran Khan tweet: ${error.message}`);
    throw error;
  }
}
...
async function main() {
  ...
  logWithTimestamp('Starting Imran Khan Tweet Retweet Script');
  logWithTimestamp('='.repeat(50));
  ...
  try {
    ...
    // Retweet an Imran Khan tweet
    await retweetImranKhanTweet(browser);
